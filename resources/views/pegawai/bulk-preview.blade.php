        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 text-sm">
                                    @foreach ($invalidRows as $row)
                                        <tr class="align-top">
                                            ...
                                            <td class="px-4 py-3 text-gray-900 font-medium">{{ $row['nopeg'] }}</td>
                                            <td class="px-4 py-3 text-gray-900">{{ $row['nama'] }}</td>
                                            <td class="px-4 py-3 text-gray-900">{{ $row['nip'] }}</td>
                                            <td class="px-4 py-3">
            ...
            <div class="flex items-center justify-between">
                <a href="{{ route('pegawais.index') }}" class="text-sm text-gray-600 hover:text-gray-900">
                    {{ __('Kembali ke daftar pegawai') }}
                </a>

                @if ($token && count($validRows) > 0)
                    <form method="POST" action="{{ route('pegawais.bulk.confirm') }}" class="flex flex-col items-end gap-2 sm:flex-row sm:items-center">
                        @csrf
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Nama') }}</th>
                                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('NIP') }}</th>
                                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Tanggal Lahir') }}</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 text-sm">
                                    @foreach ($validRows as $row)
                                        <tr>
                                            <td class="px-4 py-3 text-gray-600">{{ $row['line'] }}</td>
                                            <td class="px-4 py-3 font-medium text-gray-900">{{ $row['nopeg'] }}</td>
                                            <td class="px-4 py-3 text-gray-900">{{ $row['nama'] }}</td>
                                            <td class="px-4 py-3 text-gray-900">{{ $row['nip'] }}</td>
                                            <td class="px-4 py-3 text-gray-900">{{ $row['tgl_lahir'] }}</td>
                                            <td class="px-4 py-3 text-gray-900">{{ $row['jabatan'] }}</td>
                                            <td class="px-4 py-3 text-gray-900">{{ $row['tanggal_menjabat'] }}</td>
                                            <td class="px-4 py-3 text-gray-900">{{ $row['unit_kerja'] }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
